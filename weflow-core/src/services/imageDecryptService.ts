...
function getStaticFfmpegPath(): string | null {
  try {
    ...
    // 方法3: resources/ffmpeg.exe（weflow-core 自带）
    const resPath = join(process.cwd(), 'resources', 'ffmpeg.exe')
...
export class ImageDecryptService {
  ...
  private async decryptImageInternal(
    ...
    try {
      ...
      console.error(`[img-diag] before unwrapWxgf len=${decrypted.length} isWxgf=${nativeResult.isWxgf}`)
      const wxgfResult = await this.unwrapWxgf(decrypted)
      decrypted = wxgfResult.data
      console.error(`[img-diag] after unwrapWxgf len=${decrypted.length} isWxgf=${wxgfResult.isWxgf}`)

      const detectedExt = this.detectImageExtension(decrypted)
      console.error(`[img-diag] detectedExt=${detectedExt}`)
  ...
  private tryDecryptDatWithNative(
    ...
    console.error(`[img-diag] tryDecryptDatWithNative datPath=${datPath} xorKey=${xorKey} aesKey=${aesKey}`)
    const result = decryptDatViaNative(datPath, xorKey, aesKey)
    console.error(`[img-diag] decryptDatViaNative result=${result ? 'OK '+result.ext : 'NULL'}`)
  ...
  private async unwrapWxgf(buffer: Buffer): Promise<{ data: Buffer; isWxgf: boolean }> {
    ...
    console.error(`[img-diag] unwrapWxgf: ${hevcCandidates.length} HEVC candidates`)
    ...
    for (const candidate of hevcCandidates) {
      try {
        console.error(`[img-diag] unwrapWxgf: trying ${candidate.name} (${candidate.data.length} bytes)`)
        const jpgData = await this.convertHevcToJpg(candidate.data)
        if (!jpgData || jpgData.length === 0) {
          console.error(`[img-diag] unwrapWxgf: ${candidate.name} → null/empty`)
          continue
        }
        console.error(`[img-diag] unwrapWxgf: ${candidate.name} → ${jpgData.length} bytes JPEG`)
        return { data: jpgData, isWxgf: false }
      } catch (e) {
        console.error(`[img-diag] unwrapWxgf: ${candidate.name} exception: ${(e as Error).message}`)
        this.logError('unwrapWxgf: 候选流转换失败', e, { candidate: candidate.name })
      }
    }

    console.error(`[img-diag] unwrapWxgf: ALL candidates failed, using fallback`)
  ...
  private getFfmpegPath(): string {
    ...
    console.error(`[img-diag] getFfmpegPath staticPath=${staticPath || '(null)'} exists=${staticPath ? existsSync(staticPath) : 'N/A'}`)
