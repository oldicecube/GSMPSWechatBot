  ...
  async autoStart(): Promise<void> {
    ...
    private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
        ...
        try {
            ...
            if (pathname === '/health' || pathname === '/api/v1/health') {
                this.sendJson(res, { status: 'ok', service: 'weflow-core', version: '4.3.0-headless' })
  ...
  private async exportMediaForMessage(
    ...
    try {
      if (msg.localType === 3 && options.exportImages) {
        console.error(`[http-diag] 解密图片 localId=${msg.localId} md5=${msg.imageMd5} datName=${msg.imageDatName}`)
        ...
        if (!imagePath) {
          console.error(`[http-diag] decryptImage 失败 localId=${msg.localId} success=${result.success} error=${result.error || '(无)'} failureKind=${result.failureKind || '(无)'}`)
          try {
